...
const createAlert = (text) =>
  Alert.alert('Missing some data', text, [{ text: 'OK' }], {
    cancelable: false,
  });
...
const ColorPaletteModal = ({ navigation }) => {
  ...
  const handleColorChoice = {
    add: (chosenColor) => {
      setColors((state) => [chosenColor, ...colors]);
    },
    remove: (colorName) => {
      setColors((state) => {
        const colorIndex = state.findIndex((el) => el.colorName === colorName);
        return [...state.slice(0, colorIndex), ...state.slice(colorIndex + 1)];
      });
    },
  };
  ...
  const handleCreatePalette = useCallback(() => {
    if (!paletteName) {
      return createAlert('You should provide the name for a palette');
    } else if (colors.length < 3) {
      return createAlert('The palette should consist of three colors at least');
    } else {
  ...
  return (
    <View style={{ padding: 20, backgroundColor: 'white', flex: 1 }}>
      <Text style={[styles.label, { marginBottom: 10 }]}>Palette name</Text>
      <TextInput
        style={styles.input}
        value={paletteName}
        onChangeText={setPaletteName}
        placeholder="enter the name for new color palette"
      />
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <ColorOption onChoice={handleColorChoice} {...item} />
        )}
      />
      <TouchableOpacity style={styles.button} onPress={handleCreatePalette}>
